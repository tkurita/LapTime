...
CFStringRef errorMessageWithError(OSErr err)
{
	...
	switch (err) {
		...
		case kFailToGettimeofday:
			...
			break;
		case kNoTimers:
			result = CFSTR("No timers generated by start timer.");
			break;			
		default:
			break;
	}
...
OSErr stopTimerHandler(const AppleEvent *ev, AppleEvent *reply, SRefCon refcon)
{
	...
	//Size actual_size;
	err = AEGetKeyPtr(&timer_spec, 'ID  ', typeSInt32, NULL, &timer_id, sizeof(timer_id), NULL);
	//err = AEGetKeyPtr(&timer_spec, 'ID  ', typeSInt32, NULL, &timer_id, sizeof(timer_id), &actual_size);
	//fprintf(stderr, "resuested size : %d, actual size : %d\n", sizeof(timer_id), actual_size);
	if (err != noErr) goto bail;
	if (!TIMERS) {
		err = kNoTimers;
		goto bail;
